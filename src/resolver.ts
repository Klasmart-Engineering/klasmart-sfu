...
export async function Resolver<T>() {
    return new Promise<{ promise: Promise<T>, resolver: (t: T) => unknown }>(
        (resolve) => {
            const result: { promise: Promise<T>, resolver: (t: T) => unknown } = { promise: undefined, resolver: undefined } as any;
