...
import {MuteNotification} from "./types";
import newrelic from "newrelic";
import {Consumer} from "mediasoup/node/lib/Consumer";
import {Logger} from "./logger";
import {PubSub} from "graphql-subscriptions";
import {KidsloopLiveAuthorizationToken} from "kidsloop-token-validation";
...
export class Client {
    ...
    public teacherAudioMuted = false;
    public teacherVideoDisabled = false;
    ...
    private constructor(
        ...
        this.producerTransport.on("routerclose", () => {
            this.channel.publish("close", {media: {close: producerTransport.id}}).catch((e: unknown) => Logger.error(e));
        });
        this.destructors.set(producerTransport.id, () => producerTransport.close());

        this.consumerTransport.on("routerclose", () => {
            this.channel.publish("close", {media: {close: consumerTransport.id}}).catch((e: unknown) => Logger.error(e));
        });
    ...
    public static async create(
        ...
        jwt: KidsloopLiveAuthorizationToken) {
    ...
    public static async forwardStream(stream: Stream, source: Client, destination: Client) {
        ...
        const promises = producers.map(async (producer) => {
            ...
            if(!consumerSet) {
                const errorMessage = `Unable to find producer to consumer mapping from Producer(${producer.id})`;
                Logger.crit(errorMessage);
            }
        });
